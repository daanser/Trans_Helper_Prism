...
> 由 `plan.md` v0.2 拆出。决策基线（2026-09-07，2026-09-09 修订配额）：GPL-3.0 沿用；配额为滚动 5 小时窗口 + 加权 token 额度；未登录/超额走关键词回退+登录引导；
...
- OAuth（X 优先，可选加 GitHub；Workers 内 `arctic` 类库）+ `jose` JWT（短 access + refresh）；账号↔绑定↔封禁表放 D1；
  未绑定只能关键词回退；**隐私条目逐项落实**：X 只取 id+username 且**只落 `sha256(x_id)`**（不存 x_id/handle 明文）、登录页明示用途、可解绑、不存邮箱/实名/手机、日志与后台脱敏。
- ✅ 已线上验证（2026-09-09）：登录→建号→JWT→`/me` 全通；DB 只存 `sha256(x_id)`；管理后台看到的是脱敏 id。

### T3.2 配额（滚动 5 小时窗口 + 加权 token）计量扣减
- **2026-09-09 修订（用户澄清）**：不是「每月 5h」，而是**滚动 5 小时窗口 + 固定额度**（同 ChatGPT / Gemini）：
  每账号记 `window_start`，满 5h 自动开新窗口；额度来自 env `QUOTA_WINDOW_TOKENS`（默认 300,000 加权 token/窗口）。
- 权重表（初值，M4 可调）：纯搜索 200、+rerank +100、LLM 按上游真实 `tokens_in+tokens_out`、回退 0。
- 扣减走 **D1 原子 UPDATE**（KV 无原子自增，不用于记账）；返回 `used_pct` / `remaining_pct`（0–100，1 位小数），
  **前端只显示百分比**；管理员可加/重置当前窗口额度。
- 存储零 DDL：复用 `quotas` 表（`period_start`→`window_start`、`used_cost`→`used_tokens`）。
