...
- **限流重做（2026-09-09 定稿方向）**：反代后 KV 限流实测失效（跨 colo 不收敛）、且 Worker 看不到真实客户端 IP/ASN，
  改为「CF 边缘规则 + Worker 内 D1 分档计数」两层；分档：CN 家宽 30 / CN 其它 15 / 云与 IDC 10 / 境外 10 / 登录 60 次每分钟。
  **完整设计见 [`plan-ratelimit.md`](./plan-ratelimit.md)**（含 ASN 清单维护方式、HMAC 不存 IP、熔断阈值、5h 窗口网格锚定与重置展示）。
