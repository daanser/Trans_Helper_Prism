...
## 🔴 进行中：开业酬宾（限时 DeepSeek V4.1 Flash）
- **设计稿：[`plan-promo.md`](./plan-promo.md)**（两把 ¥68 key、仅登录用户、思考等级可调、4× 额度、用完即止）
- **下一步是"步 0 探测"**：用真实请求探明 TokenRhythm 对 `deepseek-flash` 的**思考参数到底能不能分级**
  （本仓库已实测过同类参数在国内网关**被静默忽略**，不能假设），以及流式 usage / 余额端点 / 错误码形态。
- ⏳ 需要你：① 两把 key（CF Secrets `DS_POOL_KEY_0/_1`）② **单价**（¥/M 输入·输出）③ 拍板 plan §9 的 7 个问题。
